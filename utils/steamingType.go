...
// 某時間搓 多筆資料平均 也是插入db的最小單位
type TradingData struct {
	Symbol      string
	Price       decimal.Decimal
	Volume      decimal.Decimal
	InsRate     int32 // c 含有8的比例
	NextRate    int32
	EventNum    int32
...
func (self *TradingData) GetField() {
	...
	self.InsRate = int32(insvol.Div(self.Volume).Mul(decimal.NewFromInt(100)).IntPart())
	self.NextRate = int32(nextvol.Div(self.Volume).Mul(decimal.NewFromInt(100)).IntPart())
